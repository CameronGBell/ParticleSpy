...
def find_zoneaxis_gen(im, tit):
    ...
    # Create plain mask
    grad = np.ones_like(im)*255

    # Create radial alpha/transparency layer. 255 in centre, 0 at edge
    Y = np.linspace(-1.414/2, 1.414/2, grad.shape[1])[None, :]*255
    X = np.linspace(-1.414/2, 1.414/2, grad.shape[0])[:, None]*255
    
    alpha = np.sqrt(X**2 + Y**2)
    alpha = np.clip(0,255,alpha)

    rad_norm = fft_rescaled*alpha
    rad_norm = np.uint8(exposure.rescale_intensity(rad_norm, in_range=(0,np.max(rad_norm)), out_range='uint8'))
    plt.imshow(rad_norm, cmap = 'gray')
    plt.title(tit)
    plt.show()

    selem = morphology.disk(5)
    fft_rolled = rolling_ball(rad_norm,2)
    fft_smoothed = filters.rank.maximum(fft_rolled, selem)
...
if __name__ == '__main__':
    ...
    from particlespy.segptcls import process, rolling_ball
    
    path = f"C:/Users/cimca/Google Drive (cameronbell2236@gmail.com)/Diamond/simulations/NT"
