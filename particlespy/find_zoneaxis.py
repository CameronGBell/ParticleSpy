...
def find_zoneaxis_gen(im):
    
    imfile = hs.load(os.path.join(path,image))
    im = imfile.data

    fft = fftim.fft2(im)
    fshift = fftim.fftshift(fft)
    fshift_real = np.abs(np.real(fshift))

    #Subtract central area
    circle = set_circle(im.shape,int(im.shape[0]/9))
    fshift_real[circle==True] = 0
    ...
    fft_rolled = rolling_ball(fft_rescaled,2)
    fft_smoothed = filters.gaussian(fft_rolled,sigma)

    #plott = plt.figure()
    plt.imshow(fft_smoothed)
    plt.show()

    #this threshold will need to be generalised?

    params = ps.parameters(threshold='lg_otsu',watershed=False,watershed_size=0,
                 watershed_erosion=0,invert=False,min_size=0,store_im=False,
                 pad=5,rb_kernel=0,gaussian=2,local_size=13)
    
    
    
    fshift_real[circle==True] = 0
    hpspy = hs.load("C:/Users/cimca/Google Drive (cameronbell2236@gmail.com)/Diamond/Images Rep/AuGe TEM/image/300kX _0226.tiff")
    hpspy.data = fshift_real
    fft_thresh = process(hpspy,params)
    fft_thresh[fft_thresh != 0 ] = 1


    hpspy = hs.load("C:/Users/cimca/Google Drive (cameronbell2236@gmail.com)/Diamond/Images Rep/AuGe TEM/image/300kX _0226.tiff")
    hpspy.data = fft_smoothed
    
    #ps.seg_ui(hpspy)


    #value to segment above
    #thresh_fft = fft_smoothed.max()/20
    #thresh_fft = fft_smoothed[circle==True].mean()
    #fft_thresh = fft_smoothed > thresh_fft
    ...
    for j,prop in enumerate(properties):
        ...
        max_ind.append(np.where(fft_smoothed[markers==j] == prop.max_intensity))

        distances.append(np.sqrt((centroids[j][0]-int(im.shape[0]/2))**2+(centroids[j][1]-int(im.shape[0]/2))**2))

        vectors.append((centroids[j][0]-im.shape[0]/2,centroids[j][1]-im.shape[0]/2))

        areas.append((prop.area))
        #print(prop.max_intensity)
        #print(vectors[x],distances[x],max_ind[x],centroids[x])
